...
const LeadManagerDashboard = () => {
    ...
    const handleContactLead = async (leadId) => {
        try {
            const response = await apiCall(`/leads/${leadId}`, {
                method: 'PATCH',
                body: JSON.stringify(
                    {status: 'contacted', lastContact: new Date().toISOString(), notes: 'Lead contacted by phone/email'}
                )
            });
        ...
        if (window.confirm(`Mark "${leadName}" as qualified?\n\nThis will make the lead available for CRM assignment.`)) {
            try {
                const response = await apiCall(`/leads/${leadId}`, {
                    method: 'PATCH',
                    body: JSON.stringify(
                        {status: 'qualified', lastContact: new Date().toISOString(), notes: 'Lead qualified by Lead Manager'}
                    )
                });
    ...
    const handleShowAnalytics = () => { // Create analytics modal
        const modalHTML = `
            ...
            if (response.success) {
                ...
                if (lead.convertedToProject) {
                    const projectResponse = await apiCall(`/projects/${
                        lead.convertedToProject._id
                    }`);
                    if (projectResponse.success) { // Show project details modal
        ...
        switch (activeSection) {
            case 'dashboard':
                return (<>
                    ...
                    <QuickActions onConvertToProject={
            ...
            case 'leads':
                return (<>
                    ...
                    <LeadsTable leads={leads}
                        ...
                        onViewProjectDetails={handleViewProjectDetails}/> {/* Pagination */}
                    {
                    pagination.totalPages > 1 && (<div className="flex justify-between items-center mt-6">
                        ...
                        <div className="flex gap-2"> {
                            pagination.page > 1 && (<button onClick={
                                    () => loadLeads(pagination.page - 1)
                                }
                            ...
                            {
                            pagination.page < pagination.totalPages && (<button onClick={
                                    () => loadLeads(pagination.page + 1)
                                }
                                className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">
                                Next
            ...
            case 'projects':
                return (<ProjectsManagement projects={
                        []
                    }
                    clients={
                        []
    ...
    return (<div className="min-h-screen"
        ...
        {/* Modals */}
        <ConvertToProjectModal isOpen={convertModalOpen}
            onClose={
                () => {
        ...
        <AssignProjectsToCrmModal isOpen={assignModalOpen}
            onClose={
        ...
        <LeadDetailsModal lead={
                leadDetailsModal.lead
            }
            isOpen={
                leadDetailsModal.open
            }
            onClose={
                () => setLeadDetailsModal({open: false, lead: null})
            }
            onContactLead={handleContactLead}
            onQualifyLead={handleQualifyLead}
            onConvertToProject={handleConvertToProject}/>
