...
import { projectsAPI, appointmentsAPI } from '../../../../services/api';
...
const ProjectsManagement = ({ user }) => {
  ...
  const loadAssignedProjects = async () => {
    try {
      setLoading(true);
      console.log('🔄 Loading assigned projects for CRM manager...');
      console.log('🔄 Making API call to:', '/api/projects/my-projects');
      
      // Get only projects assigned to current CRM manager
  ...
  const viewClientDetails = (project) => {
    setModalData(project);
    setModalType('client');
    setShowModal(true);
  };
...
const ProjectDetailsModal = ({ show, onHide, type, data, onUpdateStatus, user }) => {
  ...
  const [meetingData, setMeetingData] = useState({
    title: `Meeting with ${data?.client?.name || 'Client'}`,
    date: '',
    time: '',
    duration: '60',
    type: 'video', // Use valid consultation type
    agenda: '',
    notes: '',
    meetingLink: ''
  });
  ...
  const handleScheduleMeeting = async () => {
    if (meetingData.date && meetingData.time) {
      try {
        ...
        // Simplified appointment data that matches the expected API format
        const appointmentData = {
          ...
          preferred_time: meetingData.time,
          consultation_type: meetingData.type, // Use the selected type directly
          details: meetingData.agenda || `Meeting regarding project: ${data.service?.name || data.service_name}`,
          status: 'confirmed', // Use 'confirmed' like lead manager
          priority: 'medium',
          assigned_to: user?._id || user?.id, // Assign to current user
          created_by: user?._id || user?.id, // Use actual user ID
          scheduled_date: new Date(`${meetingData.date}T${meetingData.time}`),
          scheduled_time: meetingData.time,
          duration_minutes: parseInt(meetingData.duration) || 60,
          meeting_link: meetingData.meetingLink || '',
          consultation_notes: `Meeting scheduled by CRM Manager for project: ${data.service?.name || data.service_name}`
        };
        
        console.log('🔄 Scheduling meeting with simplified data:', appointmentData);
        
        // Call the appointments API to create the meeting
        const response = await appointmentsAPI.submit(appointmentData);
        
        console.log('✅ Meeting scheduled successfully:', response);
        
        if (response.success) {
          alert('Meeting scheduled successfully! You can view it in the Appointments section.');
          onHide();
        } else {
          ...
          <div className="modal-body" style={componentStyles.modalBody}>
            {type === 'client' ? (
              <div>
                ...
                <div className="row">
                  <div className="col-md-6">
                    ...
                    <p><strong>Company:</strong> {data.client?.company || 'N/A'}</p>
                  </div>
                  <div className="col-md-6">
                    <p><strong>University:</strong> {data.client?.university || 'N/A'}</p>
                    <p><strong>Country:</strong> {data.client?.country || 'N/A'}</p>
                    <p><strong>Status:</strong> {data.client?.status || 'N/A'}</p>
                    <p><strong>Registration Date:</strong> {data.client?.createdAt ? new Date(data.client.createdAt).toLocaleDateString() : 'N/A'}</p>
                  </div>
                </div>
              ...
              <div>
                {/* Client Information */}
                <div className="mb-4">
                  <div className="card border-0" style={{ background: designSystem.colors.gray[100], padding: designSystem.spacing.md }}>
                    ...
                    <div className="row">
                      ...
                      <div className="col-md-6">
                        <div className="mb-2">
                          <strong>Phone:</strong> {data.client?.phone || 'Not provided'}
                        </div>
                        <div className="mb-2">
                          <strong>Project:</strong> {data.service?.name || data.service_name || 'N/A'}
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="card border-0" style={{ background: '#e0f2fe', padding: designSystem.spacing.md }}>
                      ...
                      <div className="row">
                        ...
                        <div className="col-md-6">
                          <div className="mb-2">
                            <strong>Type:</strong> {
                              meetingData.type === 'video' ? 'Video Call (Zoom/Teams)' :
                              meetingData.type === 'phone' ? 'Phone Call' :
                              meetingData.type === 'in-person' ? 'In-Person Meeting' :
                              meetingData.type.toUpperCase()
                            }
                          </div>
                          <div className="mb-2">
                            <strong>Attendees:</strong> You, {data.client?.name || 'Client'}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div>
                ...
                {/* Client Details in Project View */}
                <div className="mt-4">
                  <h6 style={{ color: designSystem.colors.dark, marginBottom: designSystem.spacing.md }}>
                    Client Details
                  </h6>
