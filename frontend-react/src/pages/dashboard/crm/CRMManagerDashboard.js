...
const CRMManagerDashboard = () => {
  ...
  const loadMyClients = async () => {
    try {
      const [clientsResponse, leadsResponse] = await Promise.all([
        apiCall('/clients/my-clients').catch(() => ({ success: false, data: [] })),
        apiCall('/leads/my-leads').catch(() => ({ success: false, data: [] }))
      ]);
      ...
      const allClients = [...clients, ...leadsAsClients];
      
      setDashboardData(prev => ({ ...prev, clients: allClients }));
      setStats(prev => ({
        ...prev,
        totalClients: allClients.length,
        clientsCount: allClients.length,
        clientsChange: leads.length > 0 ? `${leads.length} new leads` : 'All converted'
      }));
    } catch (error) {
    ...
    try {
      ...
      if (response.success) {
        ...
        setStats(prev => ({
          ...
          projectsChange: `${projects.length} total projects`,
  ...
  const loadMyTasks = async () => {
    try {
        ...
        if (project.milestones && project.milestones.length > 0) {
          project.milestones.forEach(milestone => {
            const client = dashboardData.clients.find(c => c._id === project.client) || 
              { name: 'Unknown Client', _id: project.client };
      ...
      setDashboardData(prev => ({ ...prev, tasks: allTasks }));
      setStats(prev => ({ ...prev, tasksCount: pendingTasks }));
    } catch (error) {
  ...
  const loadClientQueries = async () => {
    try {
      ...
      if (response.success) {
        ...
        setDashboardData(prev => ({ ...prev, queries }));
        setStats(prev => ({
          ...prev,
          pendingQueries,
          queriesCount: pendingQueries,
          queriesChange: `${queries.length} total queries`
        }));
      }
    } catch (error) {
  ...
  const loadAppointments = async () => {
    try {
      ...
      setDashboardData(prev => ({ ...prev, appointments }));
      setStats(prev => ({ ...prev, appointmentsCount: upcomingAppointments }));
    } catch (error) {
  ...
  const loadPayments = async () => {
    try {
      ...
      setDashboardData(prev => ({ ...prev, payments: foundPayments }));
    } catch (error) {
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
      case 'dashboard':
        return <CrmStats stats={stats} recentActivity={dashboardData.recentActivity} />;
      case 'clients':
        return <ClientsManagement apiCall={apiCall} onViewClient={() => {}} onMessageClient={() => {}} />;
      case 'projects':
        return <ProjectsManagement apiCall={apiCall} onViewProject={() => {}} onUpdateProject={() => {}} />;
      case 'tasks':
        return <TasksManagement apiCall={apiCall} projects={dashboardData.projects} clients={dashboardData.clients} />;
      case 'queries':
        return <QueriesManagement apiCall={apiCall} clients={dashboardData.clients} />;
      case 'appointments':
        return <AppointmentsManagement apiCall={apiCall} clients={dashboardData.clients} />;
      case 'payments':
        return <PaymentsManagement apiCall={apiCall} clients={dashboardData.clients} />;
      default:
        return <CrmStats stats={stats} recentActivity={dashboardData.recentActivity} />;
    }
  };
