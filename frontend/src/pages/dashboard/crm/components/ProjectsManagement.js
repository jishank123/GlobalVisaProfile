...
import { projectsAPI, appointmentsAPI, clientsAPI } from '../../../../services/api';
  ...
  const loadAssignedProjects = async () => {
    try {
      ...
      if (response && response.success) {
        ...
        // Debug: Log each project's client information
        response.data?.forEach((project, index) => {
          console.log(`📊 Project ${index + 1} Client Info:`, {
            projectId: project._id,
            project_id: project.project_id,
            clientId: project.client?._id,
            clientName: project.client?.name,
            clientFirstName: project.client?.firstName,
            clientLastName: project.client?.lastName,
            clientEmail: project.client?.email,
            serviceName: project.service?.name || project.service_name
          });
        });
