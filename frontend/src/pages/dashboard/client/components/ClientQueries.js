...
const ClientQueries = ({ clientData, apiCall, onRefresh }) => {
  ...
  const loadQueries = async () => {
    setLoading(true);
    try {
      // Load queries for this client
      console.log('🔄 Loading queries for client:', clientData?.email);
      const response = await apiCall('/queries?client=' + clientData?.email);
      
      if (response.success) {
        console.log('✅ Queries loaded for client:', response.data?.length || 0, 'queries');
      ...
      if (error.message.includes('permission') || error.message.includes('403')) {
        console.warn('Permission denied for queries - client may not have access');
